...
export function ActivityDetail({ activity, saved, joining, joined, canJoin, onBack, onSave, onJoin, onOpenParticipant }: { activity: Activity; saved: boolean; joining: boolean; joined: boolean; canJoin: boolean; onBack: () => void; onSave: () => void; onJoin: () => void; onOpenParticipant: (person: Person) => void }) {
  ...
  return <article className="detail-page detail-page--activity" aria-labelledby="activity-detail-title" data-screen-label="活动详情">
    ...
    <div className="detail-body">
      ...
      <section className="detail-section"><div className="section-title"><h2>一起去的人</h2><span>仅已确认并授权公开</span></div><div className="people-strip">{activity.visibleParticipants.map((participant) => { const person = findPersonById(participant.personId); return person ? <button type="button" key={participant.personId} aria-label={`查看参与者：${person.displayName}`} onClick={() => onOpenParticipant(person)}><SafeImage src={person.photos[0].url} alt={person.displayName} ratio="1"/><b>{person.displayName}</b><span>{participant.role === 'ORGANIZER' ? '发起者' : '已确认'}</span></button> : <div key={participant.personId}><span className="avatar-fallback">友</span><b>已确认成员</b><span>{participant.role === 'ORGANIZER' ? '发起者' : '已确认'}</span></div>; })}</div></section>
...
export function PersonDetail({ person, suggestedActivity, hearted, hearting, onBack, onHeart, onOpenActivity, onStartConversation }: { person: Person; suggestedActivity: Activity; hearted: boolean; hearting: boolean; onBack: () => void; onHeart: () => void; onOpenActivity: (activity: Activity) => void; onStartConversation?: () => void }) {
  ...
  return <article className="detail-page detail-page--person" aria-labelledby="person-detail-title" data-screen-label="用户详情">
    ...
    <div className="detail-body">
      ...
      <section className="safety-panel"><LockKeyhole/><div><h2>低压力连接</h2><p>{onStartConversation ? '你们已确认参加同一活动，可以主动发起对话；是否回复由对方决定。' : '心动不会通知对方。只有双方都表达心动，才会建立一个可发消息的连接。'}</p><button type="button"><Flag size={14}/>举报或屏蔽</button></div></section>
    </div>
    <DetailActionPortal>
      <footer className={'sticky-action ' + (onStartConversation ? 'person-connect-actions' : 'sticky-action--single')} data-detail-action="person" aria-label="个人连接操作"><button className={'secondary-button heart-cta ' + (hearted ? 'is-active' : '')} disabled={hearting} onClick={onHeart}><Heart fill={hearted ? 'currentColor' : 'none'}/>{hearting ? '正在确认…' : hearted ? '已喜欢' : '喜欢'}</button>{onStartConversation && <button className="primary-button" onClick={onStartConversation}>发起对话</button>}</footer>
    </DetailActionPortal>
  </article>;
}
