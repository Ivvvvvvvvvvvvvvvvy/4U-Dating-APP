...
export default function App() {
  ...
  return <>
    ...
    <AppShell active={routeTab(route)} detail={detail} immersive={['entry','login','chat','create','search','onboarding'].includes(route.kind)} chromeless={route.kind === 'entry' || route.kind === 'login' || route.kind === 'onboarding'} viewer={dbProfile?.person ?? currentUser.profile} onNavigate={go} onCreate={() => go('/activities/new/local-draft/1')} onSwitchAccount={() => { void signOutSession(); go('/login'); }}>{page}</AppShell>
